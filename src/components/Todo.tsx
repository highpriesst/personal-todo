...
function Todo({ todo, index, handleUpdateTodo }: ITodoProps) {
  const isCompleted: string = todo.completed ? `line-through` : ``;
  return (
    <div>
      <ul className="flex mb-4 items-center">
        <li key={index}>
          <span>Title: {todo.title}</span>
          <span className={isCompleted}>Description: {todo.description}</span>
          <span>
